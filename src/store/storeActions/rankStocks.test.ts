...
import { solveHand } from "../../core/poker";
...
const pairs = [
  ...
  new Pair({ cards: [cardFromString("2s"), cardFromString("4h")] }), // #3
  new Pair({ cards: [cardFromString("2h"), cardFromString("4s")] }), // #3
  new Pair({ cards: [cardFromString("2d"), cardFromString("4c")] }), // #3
];
...
beforeEach(() => {
  ...
  solvedStocks.forEach((stock, i) => {
    stock.pair = pairs[i];
    stock.solvedHand = solveHand(new Hand({ pair: stock.pair, flop }));
  });
});

test("ranks stocks in correct order", () => {
  const mockStore = { stocks: solvedStocks } as TStore;
  ...
  expect(res.stocks[3].rank).toBe(3);
  expect(res.stocks[4].rank).toBe(3);
  expect(res.stocks[5].rank).toBe(3);
});
